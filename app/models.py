...
class DimInvestmentTerm(Base, StatusMixin):
    __tablename__ = "dim_investment_term"

    id = Column(Integer, primary_key=True, index=True)
    name = Column(String, unique=True, nullable=False)
    description = Column(Text)

    products = relationship("ProductMaster", back_populates="investment_term")
...
class ProductMaster(Base):
    ...
    investment_term_id = Column(Integer, ForeignKey("dim_investment_term.id"))
    ...
    investment_term = relationship("DimInvestmentTerm", back_populates="products")
